...
public class LoginLayoutIT extends AbstractViewTest {
  ...
  public void testBasicBehaviour() {
    ...
    assertTrue("Custom image is not present",vlow.$(ImageElement.class).attribute("alt", "Login image").exists());
    ...
    if (divSlotFormExists) {
      boolean divContainsFormLayoutExists = $(DivElement.class).attribute("slot", "form").first().$(FormLayoutElement.class).exists();
